...
const Kyatti = () => {
  const { kyattizenhan, kyattikouhan } = Data();
  useEffect(()=>{
    gsap.set('.zenhan',{y:'200px',opacity:'0',})
    gsap.to('.zenhan',{y:'0',direction:'1.5',opacity:'1',ease: 'Power2.easeOut',},'2.7')
    
    gsap.set('.kouhan',{y:'200px',opacity:'0',})
    gsap.to('.kouhan',{y:'0',direction:'1.5',opacity:'1',ease: 'Power2.easeOut',},'3.2')
      ...
      <section className="kyatti">
        <ul className="zenhan">
          {kyattizenhan.map((data, index) => (
            <li id={data.id} key={index}>{data.text}</li>
          ))}
          <div id='masuku1'/>
        </ul>
        <ul className="kouhan">
